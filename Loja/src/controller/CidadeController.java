...
public class CidadeController {
        ...
        try {
            ...
            if (!objetos.isEmpty()) {
                for (Cidade objeto : objetos) {//alterar a classe
                    ...
                    colunas[2] = objeto.getCod_estado();
    ...
    public static void atualizaCampos(CidadeView tela) {
        ...
        String cod_estado = tela.tabela.getValueAt(linhaSelecionada, 2).toString();
        
        //alterar setando os valores dos campos
        tela.jtfCodigo.setText(codigo);
        tela.jtfNome.setText(nome);
        tela.jtfCodEstado.setText(cod_estado);
    ...
    public static void adicionar(CidadeView tela) {
        ...
        Integer cod_estado= Integer.parseInt(tela.jtfCodEstado.getText().trim()); 

        //alterar:: criando objeto
        Cidade cidade = new Cidade();
        cidade.setNome(nome);
        cidade.setCod_estado(cod_estado);

        //alterar:: adicionando o objeto no banco de dados
        CidadeDAO dao = new CidadeDAO();
        boolean resultado = dao.adicionar(cidade);
        if (resultado) {
    ...
    public static void alterar(CidadeView tela) {
        //verificando se os campos estão preenchidos
        if (!verificarCampos(tela)) {
            return; //algum campo não está preenchido corretamente
        }
        ...
        //alterar:: criando objeto
        Cidade cidade = new Cidade();
        cidade.setCodigo(codigo); //na alteração tem que setar o código
        cidade.setNome(nome);
        cidade.setCod_estado(cod_estado);

        //alterar:: alterando o objeto no banco de dados
        CidadeDAO dao = new CidadeDAO(); //alterar
        boolean resultado = dao.alterar(cidade); //alterar
    ...
    public static void excluir(CidadeView tela) {
        ...
        //alterar:: criando objeto
        Cidade cidade = new Cidade();
        cidade.setCodigo(codigo); //na exclusão só precisa setar a chave primária

        //alterar:: excluindo o objeto no banco de dados
        CidadeDAO dao = new CidadeDAO(); //alterar
        boolean resultado = dao.excluir(cidade); //alterar
     ...
     */
    public static boolean verificarCampos(CidadeView tela) {
        ...
        if (tela.jtfCodEstado.getText().isEmpty()) {
            JOptionPane.showMessageDialog(tela, "Preencha o campo código estado!");
            return false;
        }
        return true;
    }
     ...
     * @param tela
     */
    public static void limparCampos(CidadeView tela) {
        //alterar:: limpando os campos
        tela.jtfCodigo.setText("");
        tela.jtfNome.setText("");
        tela.jtfCodEstado.setText("");
