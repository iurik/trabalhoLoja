...
public class VendaController {
    ...
    public static void atualizaCampos(VendaView tela) {
        ...
        String valor_total = tela.tabela.getValueAt(linhaSelecionada, 1).toString(); //está na coluna 1
        String  data= tela.tabela.getValueAt(linhaSelecionada, 2).toString();
        String  cod_cliente = tela.tabela.getValueAt(linhaSelecionada, 3).toString();
        
        //alterar setando os valores dos campos
        tela.jtfCodigo.setText(codigo);
        tela.jtfValorTotal.setText(valor_total);
        tela.jtfData.setText(data);
        tela.jtfCodCliente.setText(cod_cliente);
    ...
    public static void adicionar(VendaView tela) {
        ...
        double valor_total = Double.parseDouble(tela.jtfValorTotal.getText().trim()); 
        String data = tela.jtfData.getText().trim();
        Integer cod_cliente = Integer.parseInt(tela.jtfCodCliente.getText().trim()); 
        ...
        boolean resultado = dao.adicionar(venda);
        if (resultado) {
    ...
    public static void alterar(VendaView tela) {
        //verificando se os campos estão preenchidos
        if (!verificarCampos(tela)) {
            return; //algum campo não está preenchido corretamente
        }
        ...
        //alterar:: criando objeto
        Venda venda = new Venda();
        venda.setCodigo(codigo); //na alteração tem que setar o código
        venda.setValorTotal(valor_total);
        venda.setDatahora(data);
        venda.setCod_cliente(cod_cliente);
        ...
        VendaDAO dao = new VendaDAO(); //alterar
        boolean resultado = dao.alterar(venda); //alterar
    ...
    public static void excluir(VendaView tela) {
        ...
        //alterar:: criando objeto
        Venda venda = new Venda();
        venda.setCodigo(codigo); //na exclusão só precisa setar a chave primária

        //alterar:: excluindo o objeto no banco de dados
        VendaDAO dao = new VendaDAO(); //alterar
        boolean resultado = dao.excluir(venda); //alterar
     ...
     */
    public static boolean verificarCampos(VendaView tela) {
        //alterar:: conforme os campos obrigatórios
        if (tela.jtfValorTotal.getText().isEmpty()) {
            JOptionPane.showMessageDialog(tela, "Preencha o campo valor total!");
            return false;
        }
        if (tela.jtfData.getText().isEmpty()) {
            JOptionPane.showMessageDialog(tela, "Preencha o campo data!");
            return false;
        }
        if (tela.jtfCodCliente.getText().isEmpty()) {
            JOptionPane.showMessageDialog(tela, "Preencha o campo código cliente!");
            return false;
        }
        return true;
    }
     ...
     * @param tela
     */
    public static void limparCampos(VendaView tela) {
        //alterar:: limpando os campos
        tela.jtfCodigo.setText("");
        tela.jtfValorTotal.setText("");
        tela.jtfData.setText("");
        tela.jtfCodCliente.setText("");
